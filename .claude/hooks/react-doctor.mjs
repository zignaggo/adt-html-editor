...
const runReactDoctor = (outputPath) => {
  ...
  const localBinCandidates = process.platform === 'win32'
    ? ['node_modules\\.bin\\react-doctor.cmd', 'node_modules\\.bin\\react-doctor.exe']
    : ['./node_modules/.bin/react-doctor'];
  const localBin = localBinCandidates.find((candidate) => existsSync(candidate));
  const commands = [
    ...(localBin
