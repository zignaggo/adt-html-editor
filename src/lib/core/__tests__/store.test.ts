...
describe('store do editor', () => {
  ...
  describe('undo/redo', () => {
    ...
    it('cada ação vale uma entrada de histórico', () => {
      ...
      store.actions.setAttr(section, 'id', 'novo')
  ...
  describe('mutações', () => {
    ...
    it('duplica logo após o original', () => {
      ...
      expect(store.actions.getHtml()).toContain('<p id="p1">um</p><p id="p1">um</p>')
    })
    ...
    it('setText atualiza nós de texto', () => {
      ...
      store.actions.setText(textId, 'trocado')
      expect(store.actions.getHtml()).toContain('<p id="p1">trocado</p>')
    })
    ...
    it('replaceDocument zera histórico e seleção', () => {
      ...
      store.actions.setClasses(section, ['flex'])
      store.actions.replaceDocument('<div>novo</div>')
      expect(store.state.selectedId).toBeNull()
      expect(store.state.history.past).toHaveLength(0)
      expect(store.actions.getHtml()).toBe('<div>novo</div>')
    })
  })
