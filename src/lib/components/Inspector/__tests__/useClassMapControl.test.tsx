...
function renderControl<TValue>(html: string, classMap: ClassMap<TValue>, fallback: TValue, variant: () => VariantId) {
  ...
  const rendered = renderHook(
    () => {
      ...
      const id = h1Of(store)
      return { store, control: useClassMapControl(id, classMap, fallback, variant()) }
    },
    { wrapper },
  )
  const classesOf = () => {
...
describe('useClassMapControl', () => {
  it('reads and writes base classes', () => {
    const { control, classesOf } = renderControl('<h1 class="p-4 text-lg">Title</h1>', paddingClassMap, ZERO, () => 'base')
  ...
  it('removes classes when the value equals the fallback', () => {
    const { control, classesOf } = renderControl('<h1 class="p-4">Title</h1>', paddingClassMap, ZERO, () => 'base')
  ...
  it('cascades responsive variants and reports overrides', () => {
    let variant: VariantId = 'lg'
    const { control, classesOf, rerender } = renderControl(
      '<h1 class="font-bold sm:font-medium">Title</h1>',
      fontWeightClassMap,
      'normal',
      () => variant,
    )
    ...
    expect(classesOf()).toEqual(['font-bold', 'sm:font-medium'])

    act(() => control().setValue('light'))
    expect(classesOf()).toEqual(['font-bold', 'sm:font-medium', 'lg:font-light'])
    expect(control().override).toMatchObject({
      variant: 'lg',
      classes: ['lg:font-light'],
      fallbackVariant: 'sm',
      fallbackClasses: ['sm:font-medium'],
    })
    ...
    expect(classesOf()).toEqual(['font-bold', 'sm:font-medium', 'hover:font-black'])
  })

  it('drops classes for values that map to nothing', () => {
    const { control, classesOf } = renderControl('<h1 class="opacity-50">Title</h1>', opacityClassMap, 100, () => 'base')
